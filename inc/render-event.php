<?php
// Faqat Event arxivida JS ulash
add_action( 'wp_enqueue_scripts', function () {
    if ( is_post_type_archive( 'event' ) ) {
        wp_enqueue_script(
            'core-event-archive',
            get_template_directory_uri() . '/assets/js/event-archive.js',
            [ 'jquery' ],
            '1.0.0',
            true
        );
        wp_localize_script( 'core-event-archive', 'CoreEvent', [
            'ajax_url' => admin_url( 'admin-ajax.php' ),
            'nonce'    => wp_create_nonce( 'core_event_nonce' ),
        ] );
    }
} );

add_action( 'wp_ajax_core_filter_event',        'core_ajax_filter_event' );
add_action( 'wp_ajax_nopriv_core_filter_event', 'core_ajax_filter_event' );

function core_ajax_filter_event() {
    check_ajax_referer( 'core_event_nonce', 'nonce' );
    $year = isset( $_POST['year'] ) ? sanitize_text_field( $_POST['year'] ) : 'all';
    $html = core_event_render_items_html( $year );
...
function core_event_get_published_timestamp( $post_id = null ) {
...
function core_event_get_years() {
    ...
    foreach ( $q->posts as $pid ) {
        $years[] = (int) wp_date( 'Y', core_event_get_published_timestamp( $pid ) );
    }
    wp_reset_postdata();
...
function core_event_render_items_html( $year = null ) {
    ob_start();
    ...
    if ( $q->have_posts() ) :
        while ( $q->have_posts() ) : $q->the_post();
            $ts        = core_event_get_published_timestamp();
            ...
            ?>
            <div class="item event-item" data-year="<?php echo esc_attr( $post_year ); ?>">
                <div class="item-top">
